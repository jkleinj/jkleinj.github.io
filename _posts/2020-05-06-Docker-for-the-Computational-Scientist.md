...
Here we initialise a container *via* the image ID and additionally specify a user ID, password and port for RStudio (and Shiny). RStudio is automatically started in the container and accessible in a browser under http://localhost:8787. Do not stop this process until the modified container has been committed, see below.
```
